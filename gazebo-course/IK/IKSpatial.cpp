...
public: IKSpatialPlugin()
  ...
  // gets the number of degrees-of-freedom of a model in Gazebo (this is a hack!)
...
private: Ravelin::MatrixNd CalcJacobianNumerical(double q[6])
  {
    ...
    {
      q[i] = q[i] + DQ;
      J.column(i) = CalcOSDiff(x, FKin(q));
      q[i] = q[i] - DQ;
...
private: Ravelin::VectorNd CalcOSDiff(const math::Pose& xcurrent, const math::Pose& xdes)
  {
    ...
    math::Matrix3 dx_rot = (Rdes * RT) - math::Matrix3::IDENTITY;
    dx[3] = (.5) * (dx_rot[2][1] + dx_rot[1][2]);
    dx[4] = (.5) * (dx_rot[2][0] + dx_rot[0][2]);
    dx[5] = (.5) * (dx_rot[1][0] + dx_rot[0][1]);
...
private: void DoIK(const math::Pose& target)
  {
    ...
    const int ITER_BREAK_POINT = 1000;
    ...
    while (true)
    {
      ...
      Ravelin::MatrixNd J = CalcJacobianNumerical(theta);
      ...
      // TODO: compute f(q + dq2)
      math::Pose xprime = FKin(q);
      ...
      if (_dq.norm() < LOCAL_MIN_TOL || iter > ITER_BREAK_POINT)
      {
        if (iter > ITER_BREAK_POINT)
          std::cout << "-- hit a local minimum: norm dq (" << _dq.norm() << "), norm dx (" << DX_NORM << ") " << std::endl << " -- resetting joint angles and trying again" << std::endl;
        else
          std::cout << "-- hit too many iterations ( << " << iter << " iterations) -- resetting joint angles and trying again" << std::endl;
        for (unsigned i = 0; i < NJOINTS; i++)
          theta[i] = randD();
        restarts++;
        iter = 0;
