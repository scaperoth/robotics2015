...
private: double total_restarts;
private: double avg_restarts;
private: double total_iter;
private: double avg_iter;
...
private: math::Vector3 CalcOSDiff(const math::Pose& xcurrent, const math::Pose& xdes)
  {
    ...
    double theta_current = atan2(Tcurrent[1][0], Tcurrent[0][0]);
    double theta_des = atan2(Tdes[1][0], Tdes[0][0]);

    // TODO: compute the difference in angles
    double theta_diff = theta_des - theta_current;
...
private: void DoIK(const math::Pose& target)
  {
    ...
    Ravelin::VectorNd q(3);
    Ravelin::VectorNd _dq_t;
    Ravelin::VectorNd q_dq_t;
    ...
    while (true)
    {
      ...
      Ravelin::MatrixNd J = CalcJacobian(theta1, theta2, theta3);
      ...
      math::Pose xprime = FKin(q[0] + _dq_t[0], q[1] + _dq_t[1], q[2] + _dq_t[2]);
      ...
      while (0.5 * deltax_prime.GetLength() > 0.5 * deltax.GetLength() + ALPHA * t * grad.dot(_dq))
      {
        ...
        _dq_t = _dq;
        _dq_t *= t;
        xprime = FKin(q[0] + _dq_t[0], q[1] + _dq_t[1], q[2] + _dq_t[2]);
      ...
      // point
      if (_dq.norm() < LOCAL_MIN_TOL || iter > 20)
      {
...
public: void OnUpdate(const common::UpdateInfo & /*_info*/)
  {
    ...
    // setup the target
    if (num_sims < MAX_SIMS && first_time)
    {
      if ((int)(total_iter) % (int)(MAX_SIMS * .02) == 0) {
        std::cerr << "=";
        usleep(100);
      }
    ...
    if (num_sims >= MAX_SIMS && first_time) {
      std::cout << "|" << std::endl;
      avg_restarts = total_restarts / MAX_SIMS;
      avg_iter = total_iter / MAX_SIMS;

      std::cout << "*********************************** " << std::endl;
      std::cout << "*           FINISHED              * " << std::endl;
      std::cout << "*********************************** " << std::endl;
      std::cout << "IK solution found after " << avg_restarts << " avg restarts and " << avg_iter << " avg iterations" << std::endl;
      first_time = false;
    }
  }
